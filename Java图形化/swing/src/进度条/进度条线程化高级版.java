...
//   重点!
    BoundedRangeModel  boun=jProgressBar.getModel();
// BoundedRangeModel:使用组件的数据模型
      private void init(){
//
...
//结束定时刷新任务
//          监听进度条
          boun.addChangeListener(new ChangeListener() {
               @Override
               public void stateChanged(ChangeEvent e) {
//                   获取进度条当前任务量
                   int value = boun.getValue();
//                   进度当前任务量==任务总量,结束定时刷新任务
      ...
      private class SimulaeterAc implements Runnable{
          ...
          private volatile int current=0;   //当前任务完成量
